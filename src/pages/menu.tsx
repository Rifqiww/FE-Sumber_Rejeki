...
const Menu: React.FC = () => {
  ...
  const allCategoriesSelected = categories.length > 0 && categories.every(cat => selectedCategories.has(cat));
  const filteredProducts = Products.filter(product => {
    const matchesSearch = product.name.toLowerCase().includes(searchTerm.toLowerCase()) || product.description.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesCategory = selectedCategories.has('all') || selectedCategories.has(product.category);
  ...
  const isAllSelected = selectedCategories.has('all') || 
    (categories.length > 0 && categories.every(cat => selectedCategories.has(cat)));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="w-full md:w-64 flex-shrink-0">
            <div className="pr-2">
              <h2 className="font-lora font-bold text-2xl mb-4 text-primary">KATEGORI</h2>
              <div className="space-y-3">
                {/* All Products */}
                <div 
                  className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-colors ${
                    isAllSelected ? 'bg-tertiary-dark' : 'bg-white hover:bg-tertiary-dark'
                  }`}
                  onClick={() => toggleCategory('all')}
                >
                  <span className="text-primary font-inter font-medium">Semua Produk</span>
                ...
                {categories.map((category) => (
                  <div 
                    key={category}
                    className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-colors ${
                      selectedCategories.has(category) ? 'bg-tertiary-dark' : 'bg-white hover:bg-tertiary-dark'
                    }`}
                  ...
                  >
                    ...
                    <div className={`w-5 h-5 rounded-sm border-2 flex items-center justify-center flex-shrink-0 ${
                      ...
                      {selectedCategories.has(category) && <Check className="w-3 h-3 text-white" />}
                    </div>
                  </div>
                ))}
              </div>
